...
use @SSL_get0_alpn_selected[None](ssl: Pointer[_SSL] tag, data: Pointer[Pointer[U8] iso],
  len: Pointer[U32]) if "openssl_1.1.x" or "openssl_3"
...
use @SSL_has_pending[I32](ssl: Pointer[_SSL]) if "openssl_1.1.x" or "openssl_3"
use @SSL_get_peer_certificate[Pointer[X509]](ssl: Pointer[_SSL]) if "openssl_1.1.x"
use @SSL_get1_peer_certificate[Pointer[X509]](ssl: Pointer[_SSL]) if "openssl_3"
...
class SSL
  ...
  fun ref _get_peer_certificate(): Pointer[X509] =>
    ifdef "openssl_1.1.x" then
      return @SSL_get_peer_certificate(_ssl)
    end
    ifdef "openssl_3" then
  ...
  fun ref _verify_hostname() =>
    ...
    if _hostname.size() > 0 then
      let cert = _get_peer_certificate()
